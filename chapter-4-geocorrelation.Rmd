...
```{r}
library(ggmap)
#googmap <- get_googlemap(center = "Oklahoma", maptype = "satellite", zoom = 7, size = c(800,500), sensor = FALSE)

ggmap(get_googlemap(urlonly = FALSE), extent = "panel")
...
```{r}
annotations <- list(
  list(date = "2011-11-05", label = "5.6M", letter = 'A'),
  list(date = "2014-02-18", label = "Result of natural stresses", letter = 'B'),
  list(date = "2015-04-21", label = "Reversal in opinion", letter = 'C')
)
...
okquakes_weekly <- mutate(ok_quakes, week = floor_date(as.Date(time), 'week'), panel = 'Quakes') %>%
...
quakesg <- gp + geom_bar(data = okquakes_weekly, aes(x = week, y = count), stat = "identity", position = 'identity', color = 'black') +
  ...
  ggtitle("M3.0+ Earthquakes in Oklahoma by Week") + 
  datelines + datelabels
...
txt = paste(lapply(annotations, function(a){ paste(a$letter, a$label, sep = '. ') }), collapse = '\n')

p <- arrangeGrob(gasg, quakesg,
                 bottom = textGrob(label = txt, x = 0.1, 
                                         hjust = -0.0, vjust=0.1,
