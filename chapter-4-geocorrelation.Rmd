...
## Moving beyond earthquakes

In chapter 3, we tried some initial maps before moving on to histograms, which are more effective for quantifying the frequency and number of earthquakes.

But now that we're pretty well convinced that Oklahoma is facing unprecedented seismic activity, we need to examine the _locations_ of the earthquakes in order to make any kind of correlation between their occurrence and human activity. Let's revisit
...
_Sigh._ Rather than do a proper investigation and cleaning of this anomaly, I propose this (mis)use of math: for wells that have a `count` greater than 12, divide their `annual_total` by the ratio of `count/12`. For example, a well with a `count` of __24__ would have its `annual_total` divided by __2__, i.e. `24 / 2`. We'll store this value in a new column, `adjusted_total`:

```{r, warning = F, message = F}
wells_2013 <- wells_2013 %>%
  mutate(adjusted_volume = ifelse(count <= 12, annual_volume, annual_volume / (count / 12)))
...
Using the `adjusted_volume`, How close are we to Walsh's estimate of 160 million barrels a month?


```{r}
100 * (160000000 - (sum(wells_2013$adjusted_volume) / 12)) / 160000000
```
...
```{r}
100 * (160000000 - (sum(filter(wells_2013, is_OK)$adjusted_volume) / 12)) / 160000000
```
...
Perform the cleaning/averaging for calculating `adjusted_volume`:
...
```{r}
sum(filter(wells_2012, is_OK)$adjusted_volume) / 12
```
...
```{r}
100 * (sum(wells_2012$adjusted_volume) -
         sum(filter(wells_2012, is_OK)$adjusted_volume)) /
  sum(wells_2012$adjusted_volume)
```
...
```{r}
100 * (sum(wells_2011$adjusted_volume) -
         sum(filter(wells_2011, is_OK)$adjusted_volume)) /
  sum(wells_2011$adjusted_volume)
```
...
```{r}
# rename the columns
names(xl_2006_2010) <- c('API_COUNTY', 'API', 'LEASE_NAME', 'WELL_NUMBER', 'Y', 'X', 'ReportYear', 'FLUID_TYPE', 'PACKERDPTH', 'sub_annual_volume')
```

Since this data is already aggregated by year, we _could_ skil the summarization step. But -- surprise, surprise -- it turns out there are multiple annual saltwater injection records per well. And it doesn't appear that the records are just duplicates. So, rather than calculate an average, I set `adjusted_volume` to be the biggest (i.e. `max()`) value of the duplicate records:

```{r}
# filter the data
wells_2006_2010 <- xl_2006_2010 %>%
    mutate(sub_annual_volume = as.numeric(sub_annual_volume),
           API = paste(as.character(API_COUNTY), as.character(API), sep = ''), ReportYear = as.numeric(ReportYear)) %>%
    group_by(API, X, Y, ReportYear) %>%
    summarise(count = n(), annual_volume = sum(sub_annual_volume), adjusted_volume = max(sub_annual_volume))
```
...
```{r}
100 * (sum(wells_2006_2010$adjusted_volume) -
         sum(filter(wells_2006_2010, is_OK)$adjusted_volume)) /
  sum(wells_2006_2010$adjusted_volume)
```
...
```{r}
q1 <- ggplot(data = all_mapped_wells %>%
         group_by(Y, ReportYear) %>% summarize(total = sum(annual_volume)),
...
ggplot() +
  ...
  geom_point(data = filter(all_mapped_wells, annual_volume <= 360000),
             aes(x = X, y = Y),
