...
usgs_data <- mutate(usgs_data, time = as.Date(time), year = year(time)) %>%
...
```{r, histogram_us_earthquakes_only, message = F}
...
ggplot(states_quakes, aes(STUSPS)) +
   ...
   ggtitle("U.S. M3.0+ earthquakes since 1995, grouped by state")
```
...
ggplot(qcounts, aes(x1, count)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.6)) +
  ggtitle("U.S. M3.0+ earthquakes since 1995\ngrouped and sorted by states with most earthquake activity")
...
ggplot(top_quakes, aes(x = x1, fill = year < 2010)) +
 ...
 theme(axis.text.x = element_text(
   face = ifelse(levels(top_quakes$x1) =="OK","bold","plain"),
...
## TKTK Something Small Multiples
...
g + facet_wrap(~ STUSPS, ncol = 4) + theme(panel.margin = unit(c(1, 1, 1, 1), "lines")) +
  ggtitle("Annual number of M3.0+ earthquakes for\nU.S. states with most earthquake activity")
```
...
mygrid <- ggplot(pct_quakes, aes(x = factor(1), y = pct, fill = era)) +
  ...
  ggtitle("Earthquakes by time period for U.S. states\nwith most overall earthquake activity")
...
```{r, ok_vs_us_combined_from_2008_by_year, message = F}
