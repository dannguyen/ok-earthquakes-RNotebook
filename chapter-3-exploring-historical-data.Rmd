...
<span id="chapter-3-mark"></span>


# Chapter 3: Exploring the Historical Data
...
ok_1995_to_aug_2015_by_month <- ggplot(data = filter(ok_quakes, year >= 2008),
  ...
  scale_y_continuous(expand = c(0, 0)) +
  annotate(geom = "text", x = as.Date("2011-11-01"), y = 50, size = rel(4.5), vjust = 0.0, color = "#DD6600", family = "Gill Sans MT",
...
```{r}
nov_2011_quakes <- nov_2011_quakes %>%
...
ggplot(data = nov_2011_quakes,
       aes(x = floor_date(date, 'day'))) +
...
ggplot(data = firstweek_nov_2011_quakes, aes(x = time, y = mag)) + 
  ...
  annotate("text", x = big_q_time, y = 5.6, hjust = -0.1, size = 4,
           label = "M5.6 earthquake near Prague\n10:53 PM on Nov. 5, 2011", family = "Gill Sans MT") + 
...
([This StackOverflow question was very helpful](http://stackoverflow.com/questions/20041136/how-to-avoid-ggplot-sorting-the-x-axis-while-plotting-geom-bar))


```{r, sorted_states_top_earthquakes, message = F}
