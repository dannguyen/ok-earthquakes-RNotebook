...
ggplot(data = ok_quakes, aes(year)) +
  ...
  stat_bin( data = filter(ok_quakes, year >= 2012),
            ...
            binwidth = 1, geom = "text", vjust = 1.0, size = 3.0, 
...
Oklahoma earthquakes by month, since 2009


By month TK fix the label
TKTKTKTKTK: Do it as a grid

```{r}
# why bins so skinny for month?
ok_quakes <- filter(states_quakes, STUSPS == 'OK')
ok_quakes$time <- as.Date(ok_quakes$time)
ggplot(data = ok_quakes, aes(ceiling_date(time, 'month'))) +
  scale_x_date(breaks = c(as.Date("2012-01-01")), labels = date_format("%Y")) +
  scale_y_continuous(expand = c(0, 0)) +
  geom_histogram(fill = "gray50", binwidth = 1) +
  geom_histogram(data = filter(ok_quakes, year >= 2012), binwidth = 1, fill="#990000") +
   theme(axis.text.x = element_text(angle = 90)) +

  ggtitle("Monthly earthquakes of magnitude >= 3.0 in Oklahoma since 1995")
```

```{r}
# with year

ggplot(data = ok_quakes, aes(ceiling_date(time, 'year'))) +
...
ggplot(data = ok_quakes, aes(year_month)) +
  scale_y_continuous(expand = c(0, 0)) +
  geom_histogram(binwidth = 1, fill = "gray50", width = 1) +
### OK BECAUSE 0 things are not present!!
    geom_histogram(data = filter(ok_quakes, year >= 2010), binwidth = 1, fill="#990000", width = 1) +
    geom_histogram(data = filter(ok_quakes, year >= 2011), binwidth = 1, fill="orange", width = 1) +

    geom_histogram(data = filter(ok_quakes, year >= 2012), binwidth = 1, fill="green", width = 1) +
     theme(axis.text.x = element_text(angle = 90)) +
  ggtitle("Monthly earthquakes of magnitude >= 3.0 in Oklahoma since 1995")
```
...
g <- ggplot(top_states_quakes, aes(x = as.character(year))) +
    geom_rect(data = filter(top_states_quakes, STUSPS == 'OK'), color = 'orange', fill = NA, xmin = -Inf, xmax = Inf, ymin = 0, ymax = Inf) +
...
```{r, message = F}
  ...
  scale_x_discrete(breaks = mths_ge2005[seq(1, length(mths_ge2005), 12)], labels = yrs_ge2005) + scale_fill_manual(values = c("#FF6600", "grey")) +
