...
```{r}
tbreaks = as.numeric(seq(as.Date('2008-01-01'), as.Date('2015-08-01'), '1 month'))
ggplot(data = filter(ok_quakes, year >= 2008),
       aes(x = floor_date(time, 'month')), y = ..count..) +
  # Not sure why stat_bin works but geom_histogram makes bars too skinny...
  stat_bin(breaks = tbreaks, position = "identity") +
  stat_bin(data = filter(ok_quakes, floor_date(time, 'month') == as.Date('2011-11-01')),
          breaks = tbreaks, position = 'identity', fill = 'red') +
...
ggplot(data = filter(ok_quakes, year == 2011, month(time) == 11),
  ...
  scale_y_continuous(expand = c(0, 0)) +
  ggtitle("Oklahoma M3+ earthquakes during November 2011")
```
...
g <- ggplot(top_quakes_ge_2005, aes(year)) +
    # draw orange box around oklahoma
    geom_rect(data = filter(top_quakes_ge_2005, is_OK), color = '#FF6600', fill = "#fbefe0", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, size = 0.5) +
  geom_histogram(binwidth = 1, fill = "#999999")  +
  geom_histogram(data = filter(top_quakes_ge_2005, year >= 2012), binwidth = 1, fill="#990000") +
  geom_histogram(data = filter(top_quakes_ge_2005, year == 2015), binwidth = 1, fill="#BB9999") +
  scale_x_continuous(breaks = c(2006, 2010, 2014)) +
  scale_y_continuous(expand = c(0, 0))
g + facet_wrap(~ STUSPS, ncol = 4) + theme(panel.margin = unit(c(1, 0, 0, 0), "lines"))
```
...
mygrid <- ggplot(pct_quakes, aes(x = factor(1), y = pct, fill = era)) +
  scale_fill_manual(values = c("#999999", "#666666", "#333333", "red"),  guide = guide_legend(reverse = TRUE)) +
