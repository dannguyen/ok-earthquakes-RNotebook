...
ggplot() +
  geom_polygon(data = us_map, aes(x = long, y = lat, group = group)) +
  coord_map("albersusa", lat0 = 38, latl = 42)
```
...
```{r}
states_map <- spTransform(states_map, CRS("+proj=laea +lat_0=45 +lon_0=-100 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs"))
```



Then, we remove Hawaii and Alaska, so that we can perform a transformation on them separately before merging them back into the rest of the contiguous states:

```{r}
alaska <- states_map[states_map$STUSPS == 'AK', ]
hawaii <- states_map[states_map$STUSPS == 'HI', ]

alaska <- states_map[states_map$STUSPS=="AK",]
alaska <- elide(alaska, rotate=-50)
alaska <- elide(alaska, scale=max(apply(bbox(alaska), 1, diff)) / 2.3)
alaska <- elide(alaska, shift=c(-2100000, -2500000))
proj4string(alaska) <- proj4string(states_map)

# extract, then rotate & shift hawaii
hawaii <- states_map[states_map$STUSPS=="HI",]
hawaii <- elide(hawaii, rotate=-35)
hawaii <- elide(hawaii, shift=c(5400000, -1400000))
proj4string(hawaii) <- proj4string(states_map)
states_map <- states_map[!states_map$STUSPS %in% c('AK', 'HI'), ]
states_map <- rbind(states_map, alaska, hawaii)
ggplot() +
...
Now we use the __elide()__ function that comes with the __maptools__ package:

```{r}
alaska <- elide(alaska, rotate = -50)
alaska <- elide(alaska, scale = max(apply(bbox(alaska), 1, diff)) / 2.3)
alaska <- elide(alaska, shift = c(-40, -10))
alaska@proj4string <- states_map@proj4string
# remove the old alaska
states_map <- states_map[!states_map$STUSPS %in% c('AK', 'HI'), ]
states_map <- rbind(states_map, alaska)
...
states_map <- us_map[!(us_map$STUSPS %in% non_states), ]
alaska <- states_map[states_map$STUSPS == 'AK', ]
alaska <- elide(alaska, rotate = 0)
alaska <- elide(alaska, scale = max(apply(bbox(alaska), 1, diff)) / 2.0)
alaska <- elide(alaska, shift = c(-25, 15))
alaska@proj4string <- states_map@proj4string
# Now hawaii
hawaii <- states_map[states_map$STUSPS == 'HI', ]
hawaii <- elide(hawaii, rotate = 0)
hawaii <- elide(hawaii, shift = c(20, 0))
hawaii@proj4string <- states_map@proj4string
