...
- [maptools](https://cran.r-project.org/package=maptools) - I include this for its [elide() function](https://rud.is/b/2014/11/16/moving-the-earth-well-alaska-hawaii-with-r/), which allows us to inset Alaska and Hawaii on the U.S. map.
...
In the next section, we'll tackle this problem by converting the earthquakes data into its own spatial-aware data frame and then _cross-referencing_ it with the data in `usc_map` to remove a
...
Which, among other things, makes it possibly to do aggregates of the earthquakes dataframe by `STUSPS`:
...
## Bringing back Hawaii and Alaska

I wasn't going to attempt this but while writing my latest draft of this section, I stumbled across this excellent walkthrough by Bob Rudis -- [Moving the Earth (well, Alaska & Hawaii) With R](https://rud.is/b/2014/11/16/moving-the-earth-well-alaska-hawaii-with-r/) -- that details how to inset Alaska and Hawaii on a projected map. In this section, I reapprorpiate his technique for our specific setup.

Recall that `us_map` contains the original boundary data for every state and U.S. territory. Instead of filtering out all non-contiguous states and territories, we will now just filter out non-states, such as Guam and Washington D.C.

```{r}
non_states <- c('AS', 'DC', 'GU', 'MP', 'PR', 'VI')
states_map <- us_map[!(us_map$STUSPS %in% non_states), ]
```


Instead of applying the Albers projection via `coord_map()` during the plotting action, we apply it beforehand to the `states_map` data frame:

```{r}
albers_map <- spTransform(states_map, CRS("+proj=laea +lat_0=45 +lon_0=-100 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs"))
```



Then, we pluck out the data for Hawaii and Alaska, so that we can perform a transformation on them separately before merging them back into the rest of the contiguous states:

```{r}
alaska <- albers_map[albers_map$STUSPS == 'AK', ]
hawaii <- albers_map[albers_map$STUSPS == 'HI', ]
```


We use the __elide()__ function that comes with the __maptools__ package to perform the transformations that move Alaska and Hawaii closer on the plot:

```{r}
alaska <- elide(alaska, rotate=-50)
alaska <- elide(alaska, scale=max(apply(bbox(alaska), 1, diff)) / 2.3)
alaska <- elide(alaska, shift=c(-2100000, -2500000))
proj4string(alaska) <- proj4string(albers_map)

hawaii <- elide(hawaii, rotate=-35)
hawaii <- elide(hawaii, shift=c(5400000, -1400000))
proj4string(hawaii) <- proj4string(albers_map)
```

Then we remove/replace Alaska and Hawaii in the `albers_map` data frame:

```{r}
final_map <- albers_map[!albers_map$STUSPS %in% c('AK', 'HI'), ]
final_map <- rbind(final_map, alaska, hawaii)
```


```{r}
ggplot() +
  geom_polygon(data = final_map, aes(x = long, y = lat,
                                      group = group),
                 fill = "white", color = "#444444", size = 0.1)
```


### Plotting Alaska and Hawaii earthquakes




```{r}
# TK THIS IS WHERE transformation has to be done again
# TK why longitude.1??
sp_quakes <- SpatialPointsDataFrame(data = quakes,
                          coords = quakes[,c("longitude", "latitude")])
sp_quakes@proj4string <- states_map@proj4string
albers_quakes <- spTransform(sp_quakes, CRS("+proj=laea +lat_0=45 +lon_0=-100 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs"))


xdf <- over(albers_quakes, albers_map[, 'STUSPS'])
albers_quakes$STUSPS <- xdf$STUSPS
# filter out NA values for STUSPS
albers_quakes <- albers_quakes[!is.na(albers_quakes$STUSPS), ]

# Now transform hawaii and alaska quakes
alaska_quakes <- albers_quakes[albers_quakes$STUSPS == 'AK', ]
hawaii_quakes <- albers_quakes[albers_quakes$STUSPS == 'HI', ]


alaska_quakes <- elide(alaska_quakes, rotate=-50)
alaska_quakes <- elide(alaska_quakes, scale=max(apply(bbox(alaska), 1, diff)) / 2.3)
alaska_quakes <- elide(alaska_quakes, shift=c(-2100000, -2500000))
proj4string(alaska_quakes) <- proj4string(albers_quakes)

hawaii_quakes <- elide(hawaii_quakes, rotate=-35)
hawaii_quakes <- elide(hawaii_quakes, shift=c(5400000, -1400000))
proj4string(hawaii_quakes) <- proj4string(albers_quakes)

# Then we remove/replace Alaska and Hawaii in the `albers_map` data frame:

final_quakes <- albers_quakes[!albers_quakes$STUSPS %in% c('AK', 'HI'), ]
final_quakes <- rbind(final_quakes, alaska_quakes, hawaii_quakes)
```


```{r}

ggplot(as.data.frame(final_quakes), aes(x = longitude.1, y = latitude.1)) +
  geom_polygon(data = final_map, aes(x = long, y = lat, group = group),
                   fill = "white", color = "#444444", size = 0.1) +
  geom_point(size = 3,
               alpha = 0.5,
               shape = 1,
