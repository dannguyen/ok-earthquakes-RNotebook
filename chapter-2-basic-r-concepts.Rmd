...
If you're pretty comfortable with R, you can go ahead and skip to [Chapter 3](#chapter-3-mark)
...
This is entirely optional. You can view my [ggplot theme script and copy it from here](https://github.com/dannguyen/ok-earthquakes-RNotebook/blob/master/myslimthemes.R), and then make it locally available to your scripts (e.g. `./myslimthemes.R`). Or, just don't use them when copying the rest of my code (all of the calls begin with `theme_dan...`).
...
__Note:__ In subsequent chapters, I convert the `time` attribute with the standard `as.Date()` function and use lubridate for other conversions.
