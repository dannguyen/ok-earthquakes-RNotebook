...
A natural first attempt at visualization is to make a __scatterplot__ of the earthquake data, in which each earthquake is plotted by its time along the x-axis and its magnitude on the y-axis:
...
This ends up being a bad idea. With so many earthquake events, we run into the problem of __overplotting__: Dots representing earthquakes of similar magnitude and time will overlap and obscure each other. 
...
Having too much data to sensibly plot is going to be a major recurring theme for the entirety of this walkthrough. Hopefully the examples in this chapter broaden your techniques and ways of thinking when it comes to organizing data.

### Fun with histograms

In this scenario -- and most scenarios, actually --the best tactic is to go for something __simple__: A bar chart ([histogram](http://docs.ggplot2.org/current/geom_histogram.html)) is the most straightforward way to show _how many_ earthquakes fall within a magnitude range.
