...
If you're pretty comfortable with R, you can go ahead and skip to [Chapter 3](#mark-chapter-3)
...
__Note__: In the snippet above, I use the [__coord_equal()__ function](http://stackoverflow.com/questions/12443620/why-coord-equal-doesnt-work-the-way-it-should) instead of `coord_map("albers")` -- binning adds another complexity to the projection workflow, so I leave out Albers for now.
...
ggplot() +
  ...
  geom_point(data = ok_quakes, aes(x = longitude, y = latitude),
            color = "red") +
  stat_density2d(data = ok_quakes, aes(x = longitude, y = latitude)) +
  coord_equal() +
  ggtitle("M3.0+ earthquakes in Oklahoma, August 2015 (rectangular-binned)") +
  theme_dan_map()
```
