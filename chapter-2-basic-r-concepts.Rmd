...
Alternatively, to download a specific interval of data, such as just the earthquakes for __August 2015__, use the USGS Archives endpoint:
...
To get a data frame of __contiguous U.S.-only quakes__, we combine `xdf` with `sp_quakes`. We then filter the resulting data frame by removing all rows in which `STUSPS` is `<NA>`:
...
Try it; the resulting number of rows from the methods for `old_usc_quakes` and `usc_quakes` will be the same. However, the difference in our latest approach is that `usc_quakes` has that additional `STUSPS` column:
...
```{r, message = FALSE, warning = FALSE}
ok_quakes <- usc_quakes[usc_quakes$STUSPS == 'OK',]
ggplot(usc_quakes, aes(STUSPS)) +
  geom_histogram(binwidth = 1) +
...
For example, using the `usc_quakes` data frame, here's a time series showing __earthquakes by day__ of the month-long earthquake activity in the contiguous U.S.:


```{r, message = FALSE, warning = FALSE}
ggplot(usc_quakes, aes(x = floor_date(as.Date(time), 'hour'))) +
  geom_histogram(binwidth = 1) +
  scale_x_date(breaks = date_breaks("week"), labels = date_format("%m/%d"))
```

(The gaps represent days in which no M3.0+ earthquake occured in the lower U.S.)
...
ggplot(usc_quakes, aes(x = floor_date(as.Date(time), 'hour'))) +
  geom_histogram(binwidth = 1) +
  geom_histogram(data = filter(usc_quakes, STUSPS == 'OK'), fill = "#FF6600", binwidth = 1) +
  scale_x_date(breaks = date_breaks("week"), labels = date_format("%m/%d"))
```
...
```{r, message = FALSE, warning = FALSE}
  ggplot(data = mutate(usc_quakes, week = floor_date(as.Date(time), "week")),
